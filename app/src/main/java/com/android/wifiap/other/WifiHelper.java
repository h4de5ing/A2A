...
public class WifiHelper {
    ...
    public void connectWifi(Activity activity, String ssid, String password) {
        ...
        // 需要等待WiFi开启再去连接
        new ThreadPoolExecutor(CPU_COUNT, 2 * CPU_COUNT + 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new ConnectWiFiThread()).execute(new Runnable() {
            @Override
            public void run() {
    ...
    private boolean isGrantedLocationPermission() {
        return ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_COARSE_LOCATION) + ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION) == 0;
    }
    ...
    private void stopTethering() {
        try {
            Method stopTethering = getConnectivityManager().getClass().getDeclaredMethod("stopTethering", int.class);
    ...
    private static final class ConnectWiFiThread extends AtomicInteger implements ThreadFactory {
        ...
        public Thread newThread(Runnable r) {
            ...
            t.setName(TAG + new AtomicInteger(1) + "-pool-" + POOL_NUMBER.getAndIncrement() + "-thread-");
            t.setDaemon(false);
            t.setPriority(Thread.NORM_PRIORITY);
            t.setUncaughtExceptionHandler((t1, e) -> Log.e(TAG, "Thread run threw uncaughtException throwable", e));
            return t;
        }
    }
