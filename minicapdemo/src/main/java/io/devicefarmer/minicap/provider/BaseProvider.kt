...
abstract class BaseProvider(
    private val displayId: Int, private val targetSize: Size, val rotation: Int
) : SimpleServer.Listener, ImageReader.OnImageAvailableListener {
    ...
    var frameRate: Float = Float.MAX_VALUE
        set(value) {
            ...
            Ln.i("framePeriodMs: $framePeriodMs")
            field = value
        }
    ...
    fun init(out: DisplayOutput) {
        imageReader = ImageReader.newInstance(
            getTargetSize().width, getTargetSize().height, PixelFormat.RGBA_8888, 2
        )
        clientOutput = out
    }
