...
public class VideoDecoder {
    ...
    public void decodeSample(byte[] data, int offset, int size, int flags) {
        if (mWorker != null) mWorker.decodeSample(data, offset, size, flags);
    }

    public void configure(Surface surface, int width, int height, ByteBuffer csd0, ByteBuffer csd1) {
        if (mWorker != null) mWorker.configure(surface, width, height, csd0, csd1);
    }
